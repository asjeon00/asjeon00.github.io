...
export function createRenderer(
  ...
  const tick = (currentFrame: Frame) => {
    ...
    const healthActive =
      !performanceFrame &&
      (productionActive || dustAnimating || windAnimating) &&
      qualityPreference === "auto" &&
      currentQuality() === "high" &&
      documentVisible &&
      canvasNearViewport;
    const healthSample = (rendered: boolean) => {
      autoQualityController?.recordFrame({
        ...
        workload:
          windAnimating || updateScene
            ? "interactive"
            : dustAnimating && !revealChanged
              ? "dust"
              : "interactive",
      });
    };
    ...
    if (performanceFrame || updateScene || dustMoved || revealChanged) {
      try {
        const pointerState = interaction.getPointerState();
        runtime.pointerUv = pointerState.uv;
        runtime.pointerVelocity = pointerState.velocity;
        runtime.clockTime = gpuClock?.time ?? 0;
