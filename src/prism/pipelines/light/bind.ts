...
export function bindLightGraph(
  ...
  if (graph.paper && graph.paperShadow) {
    const paperP = globalPaperController.update();
    const pUniforms = paperUniforms(
      runtime,
      globalPaperController.getControls(),
      paperP
    );
    graph.paperShadow.set({ params: pUniforms });
    graph.paper.set({ params: pUniforms });
  }
