...
fn fs_main(in: VertexOut) -> @location(0) vec4f {
  ...
  let centerOffset = in.worldPosition.xy;
  let dropletDist = length(centerOffset);
  let rimDistance = abs(dropletDist - 0.33);
  let innerBand = band(rimDistance, accent.bandCenter, accent.bandWidth);
  let frontRim = rim(rimDistance, accent.bandWidth * 0.55);
  let baseRim = band(rimDistance, 0.0, accent.bandWidth * 0.7);
