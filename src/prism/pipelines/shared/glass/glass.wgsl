...
fn traceInteriorHit(
  ...
  if (insideDirection.z < -0.0001) {
    let tBack = (params.backZ - shiftedPosition.z) / insideDirection.z;
    if (tBack > 0.0 && tBack < 10.0) {
      let exitPos = shiftedPosition + insideDirection * tBack;
      return InteriorHit(exitPos, tBack, 1u);
    }
  }
