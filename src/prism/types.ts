...
export interface GradientControls {
  /** Soft luminous spotlight/highlight color (sRGB hex). */
  readonly colorHighlight: string;
  /** Midtone slate/stone color (sRGB hex). */
  readonly colorMid: string;
  /** Deep shadow/navy background color (sRGB hex). */
  readonly colorShadow: string;
  /** Spotlight X center in scene coordinates. */
  readonly spotCenterX: number;
  /** Spotlight Y center in scene coordinates. */
  readonly spotCenterY: number;
  /** Spotlight radius scale. */
  readonly spotRadius: number;
  /** Spotlight falloff glow steepness. */
  readonly spotGlow: number;
  /** Starting horizontal coordinate where dark sweep begins. */
  readonly darkFalloffStart: number;
  /** Ending horizontal coordinate where dark sweep reaches full darkness. */
  readonly darkFalloffEnd: number;
  /** Radial vignette darkening strength. */
  readonly vignette: number;
  /** Procedural grid line spacing in scene units. */
  readonly gridSpacing: number;
  /** Procedural grid line thickness. */
  readonly gridLineWidth: number;
  /** Opacity of procedural grid lines. */
  readonly gridOpacity: number;
  /** Horizontal shift of the Baduk board grid in scene coordinates (negative moves left). */
  readonly boardOffsetX: number;
  /** Granite noise texture spatial scale. */
  readonly graniteScale: number;
  /** Contrast/roughness contribution of granite grain to albedo. */
  readonly graniteRoughness: number;
  /** Micro and large normal bump strength for granite. */
  readonly graniteNormalStrength: number;
  /** Direct light/gradient exposure multiplier. */
  readonly directExposure: number;
  /** Influence of the baked lightmap mask (0 = pure clean gradient, 1 = full lightmap). */
  readonly lightmapMix: number;
  /** Staggered line drawing animation progress (0 = none, 1 = fully drawn). */
  readonly gridDrawProgress?: number;
}

export const DEFAULT_GRADIENT_CONTROLS: GradientControls = {
...
export interface WindControls {
  /** Enable physical wind animation of the leaf shadows. */
  readonly enabled: boolean;
  /** Speed of wind flow and branch sway oscillation. */
  readonly speed: number;
  /** Amplitude of branch and twig swaying displacement. */
  readonly strength: number;
  /** Flutter/rustle frequency of individual leaves. */
  readonly rustleFreq: number;
  /** Darkness / visibility of the dappled leaf shadows on the wall. */
  readonly leafShadowStrength: number;
  /** Sunlight shimmer / glimmer through the canopy. */
  readonly leafShimmer: number;
}

export const DEFAULT_WIND_CONTROLS: WindControls = {
  enabled: true,
  speed: 0.65,
  strength: 0.50,
  rustleFreq: 1.0,
  leafShadowStrength: 0.60,
  leafShimmer: 0.0,
};
...
export interface PrismControls {
  ...
  /** Spline studio gradient and architectural grid controls. */
  readonly gradient: GradientControls;
  /** Physical wind simulation and fluttering leaf shadows. */
  readonly wind: WindControls;
  ...
  /** Camera pitch angle in degrees (overlooking tilt). */
  readonly cameraPitch?: number;
  /** Camera yaw rotation angle in degrees. */
  readonly cameraYaw?: number;
  /** Enable mouse/touch camera orbit parallax movement. */
  readonly cameraOrbitEnabled?: boolean;
...
export const DEFAULT_PRISM_CONTROLS: PrismControls = {
  ...
  gradient: DEFAULT_GRADIENT_CONTROLS,
  wind: DEFAULT_WIND_CONTROLS,
  wallColor: "#838383",
  cameraPitch: -5,
  cameraYaw: 0,
...
/** Radius of the circular meniscus droplet solid. */
export const DROPLET_RADIUS = 0.33;
/** Height of the meniscus droplet solid above the wall. */
export const DROPLET_HEIGHT = 0.20;
...
/**
 * The resting camera overlooks the Baduk / Go board from an elevated perspective.
 */
export const DEFAULT_CAMERA_YAW_DEGREES = 0;
export const DEFAULT_CAMERA_PITCH_DEGREES = -5;
export const CAMERA_YAW_DEGREES = DEFAULT_CAMERA_YAW_DEGREES;
export const CAMERA_PITCH_DEGREES = DEFAULT_CAMERA_PITCH_DEGREES;
...
/** Baduk / Go board grid specifications */
export const GO_GRID_SPACING = 0.39;
export const GO_CELL_ASPECT = 1.07;
export const GO_STAR_POINT_RADIUS = 0.014;
export const GLASS_ORB_RADIUS = 0.182;
export const GLASS_ORB_HEIGHT = 0.14;
