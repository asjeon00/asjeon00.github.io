...
/** Aligns the scene origin (0, 0) — the central bead and board center — with the DOM slot. */
export function alignProjection(
  ...
  return {
    ...
    offset: [
      ...
      1 - viewport.top - viewport.bottom,
    ],
  };
}
...
function fitsViewport(
  ...
  const maxSpanX = Math.max(Math.abs(bounds.x0), Math.abs(bounds.x1)) * 2;
  const maxSpanY = Math.max(Math.abs(bounds.y0), Math.abs(bounds.y1)) * 2;
  return maxSpanX <= availableWidth && maxSpanY <= availableHeight;
}
