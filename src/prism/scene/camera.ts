...
export function cameraView(
  ...
  const yaw = radians(yawDegrees + clamp(orbitX, -1, 1) * limit);
  const pitch = radians(pitchDegrees - clamp(orbitY, -1, 1) * limit);
...
export function wallHalfHeight(
  ...
  for (const orbitX of [-1, 0, 1]) {
    for (const orbitY of [-1, 0, 1]) {
      worst = Math.max(
        worst,
        wallCoverage(aspect, orbitX, orbitY, distance, fov, pitch, yaw)
      );
    }
  }
...
export function wallCoverage(
  ...
  const view = cameraView(aspect, orbitX, orbitY, distance, fov, pitch, yaw);
