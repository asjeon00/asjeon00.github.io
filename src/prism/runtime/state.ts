...
export function setRuntimeControls(
  ...
  const cameraChanged =
    next.cameraFov !== runtime.controls.cameraFov ||
    next.cameraPitch !== runtime.controls.cameraPitch ||
    next.cameraYaw !== runtime.controls.cameraYaw;
...
export function wallExtent(
  ...
  yaw = CAMERA_YAW_DEGREES
): readonly [number, number] {
  const halfHeight = wallHalfHeight(aspect, cameraDistance, cameraFov, pitch, yaw);
...
export function lightWallExtent(
  ...
  const extent = wallExtent(aspect, cameraDistance, cameraFov, framing, pitch, yaw);
...
function refreshCamera(runtime: PrismRuntime): void {
  const view = cameraView(
    ...
    runtime.controls.cameraYaw ?? CAMERA_YAW_DEGREES
  );
  runtime.view = {
    ...view,
...
function refreshFraming(runtime: PrismRuntime): void {
  ...
  const pitch = runtime.controls.cameraPitch ?? CAMERA_PITCH_DEGREES;
  const yaw = runtime.controls.cameraYaw ?? CAMERA_YAW_DEGREES;
  const fit = fitProjectionDistance(
    viewport,
    (distance) =>
      projectedBounds(
        framingMatrices(runtime.aspect, distance, runtime.controls.cameraFov, pitch, yaw),
        PRISM_FRAME_POINTS
      ),
...
function framingMatrices(
  ...
  for (const orbitX of [-1, 0, 1]) {
    for (const orbitY of [-1, 0, 1]) {
      matrices.push(
        cameraView(aspect, orbitX, orbitY, distance, fov, pitch, yaw).viewProjection
      );
    }
  }
  return matrices;
